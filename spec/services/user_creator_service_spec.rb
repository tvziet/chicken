...
RSpec.describe UserCreatorService, type: :service do
  ...
  describe '#call' do
    context 'when the user represents an organization' do
      context 'when the organization does not exists' do
        it 'creates a new user and organization' do
          expect { UserCreatorService.new(user_params).call }.to change { User.count }.by(1).and change { Organization.count }.by(1)
        end

        it 'can not create an user with invalid email' do
          expect { UserCreatorService.call(user_params.merge(email: '')) }.to change { User.count }.by(0).and change { Organization.count }.by(0)
        end

        it 'can not create an user with invalid password' do
          expect { UserCreatorService.call(user_params.merge(password: '')) }.to change { User.count }.by(0).and change { Organization.count }.by(0)
        end
      end
      ...
      context 'when the organization exists' do
        ...
        it 'can not create a new user' do
          expect { UserCreatorService.call(user_params) }.to change { User.count }.by(0).and change { Organization.count }.by(0)
        end
      end
    end
    ...
    context 'when the user is an individual' do
      ...
      it 'creates a new user' do
        expect { UserCreatorService.call(updated_user_params) }.to change { User.count }.by(1)
      end

      it 'can not create an user with invalid email' do
        expect { UserCreatorService.call(updated_user_params.merge(email: '')) }.to change { User.count }.by(0)
      end
    end
  end
end
